...
const Wishlist = require('./Wishlist'); // Corrected import path for Wallet model

// POST /api/wallets - Create a new wallet
router.post('/', async (req, res) => {
  try {
    const { userId, name,amonut } = req.body;
    const newWishlist = new Wishlist({ userId, name,amonut });
...
router.get('/', async (req, res) => { // Fixed the function signature to include req
  try {
    const wishlists = await Wishlist.find(); // Changed variable name to wallets
